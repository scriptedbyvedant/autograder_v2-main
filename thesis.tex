...
\begin{itemize}
  ...
  \item Enhancing the Decision-Making Process: Empowering educators with the ability to review, approve, and manually adjust any AI-generated score or feedback. This human-in-the-loop process ensures the instructor remains the final authority and provides a valuable dataset for future system fine-tuning.
...
\section{Parameter-Efficient Fine-Tuning for Educational Graders}
\label{sec:katuka_review}

Katuka et~al.~\cite{katuka2024} explore fine-tuning LLaMA-2 with QLoRA adapters to automate grading and feedback generation. Their work is notable because it demonstrates that parameter-efficient techniques can adapt open-weight models to specific course rubrics without incurring the hardware costs of full fine-tuning. The authors present workflow scripts for training adapters and report improvements in feedback relevance after fine-tuning, echoing the motivation behind the LoRA-based pipeline described in Section~\ref{sec:model_management}. Nevertheless, the study leaves several questions unanswered: it focuses on English-language essay grading only, provides limited information on evaluation metrics beyond qualitative instructor impressions, and does not integrate a retrieval layer or HITL correction loop. This thesis builds upon their QLoRA insight by embedding adapter deployment directly into the educator workflow, combining fine-tuned models with retrieval grounding, code execution, and multilingual support to produce a more holistic grading framework.
...
\begin{itemize}
  ...
  \item \textbf{From single-shot to consensus grading.} Meta-evaluations by Katuka et~al.~\cite{katuka2024} and Chu et~al.~\cite{chu2025} illustrate the limitations of single-inference pipelines, prompting the move toward multi-agent consensus (our thesis), mixture-of-experts, and peer review ensembles.
\end{itemize}
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{l l X X}
    ...
    Katuka et~al.~\cite{katuka2024} & Essay grading & QLoRA enables course-specific fine-tuning on modest hardware. & Evaluated only qualitatively; no retrieval or bias analysis. \\
...
Persistent state is managed by PostgreSQL tables that capture professor-provided rubrics, student submissions, grading outcomes, and human corrections. A concise description of the core schema—including the human-in-the-loop columns used for audit and fine-tuning—is provided in Appendix~\ref{app:impl_details}. That appendix also catalogues how FAISS indices mirror the relational data to support retrieval-augmented prompts.
...
\begin{enumerate}
  ...
  \item \textbf{Fine-Tuning (Optional)}: Advanced users can export corrections, run the Colab fine-tuning script, and redeploy adapters for a course-specific model, aligning the workflow with Section~\ref{sec:model_management}.
\end{enumerate}
...
Additional implementation details—including the Streamlit page structure, reusable backend services, and the LoRA fine-tuning assistant—are documented in Appendix~\ref{app:impl_details}. That appendix also provides step-by-step guidance for reproducing the educator workflow outside of the controlled evaluation described here.
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{l l Y}
    ...
    Local AI/ML Runtime & MLX (Apple Silicon) & Efficient on-device inference and fine-tuning for open-source LLMs. \\
...
\section{Model Management and Fine-Tuning Workflow}
\label{sec:model_management}

To keep the framework adaptive, the repository ships with a fine-tuning assistant summarised in both the architectural notes and the Educator Guide. When an instructor selects the ``Generate Training Data'' option, the backend queries \codeword{grading_results} for rows containing human corrections and writes them to \codeword{training_dataset.jsonl}. A companion Colab script (\codeword{training/colab_finetune.py}) installs dependencies, mounts the dataset, and trains LoRA adapters on top of an MLX-hosted base model (for Apple Silicon). The resulting \codeword{trained_adapters.npz} is dropped in the \codeword{training/} directory; on startup, the application looks for this artefact and, if present, loads it automatically so subsequent grading runs benefit from the custom weights. This workflow, captured in \codeword{ARCHITECTURE.md} §5.1 and the Educator Guide's Phase~3, provides a lightweight MLOps loop that keeps the model aligned with individual course expectations.
...
\begin{itemize}
  ...
  \item \textbf{Turnaround time}: Analytics dashboards showed reduced median grading time per assignment once RAG exemplars accumulated, reinforcing the value of prompt retrieval and fine-tuning.
...
\textbf{Phase 3: Fine-Tuning (Optional)}
\begin{enumerate}
  \item Use the ``Model Finetuning Assistant'' page to export corrections as \codeword{training_dataset.jsonl}.
  \item Run the supplied Colab notebook with a T4 GPU runtime, uploading the dataset when prompted.
  \item Download the resulting \codeword{trained_adapters.npz} file and place it in the application's \codeword{training/} directory.
  \item Restart the application; the adapters load automatically and inform subsequent grading sessions.
\end{enumerate}
...
\item Use the collaboration export when moderating with multiple instructors; each correction is tracked with timestamps and editor IDs.
  ...
  \item When adapters are active, note the model version banner displayed on ``Grading Result'' to verify correct deployment.
\end{itemize}
...
\bibitem{katuka2024}
L.~Katuka, et~al.
\newblock Automatic grading and feedback using LLaMA-2 with QLoRA.
\newblock \emph{International Journal of Educational Technology}, 2024.
